...
from AssetTransfer.src.handler import make_address
...
class IntKeyPayload:
    def __init__(self, verb, name, receiver, value):
        self._verb = verb
        self._name = name
        self._value = value
...
def create_intkey_transaction(verb, name, value, receiver, deps, signer):
    ...
    Args:
        ...
        value (int): the amount to set, increment, decrement, or transfer
        receiver (str) : person to whom value is to be transfered
    ...
    payload = IntKeyPayload(
        verb=verb, name=name, value=value, receiver=receiver)
...
def do_generate(args, batches, keys):
    ...
    for i in range(0, args.count):
        ...
        for _ in range(0, random.randint(1, args.max_batch_size)):
            ...
            txn = create_intkey_transaction(
                verb=random.choice(['inc', 'dec', 'transfer']),
                name=name,
