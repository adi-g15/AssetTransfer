...
from new_client import IntkeyClient
from exceptions import IntKeyCliException
from exceptions import IntkeyClientException

DISTRIBUTION_NAME = 'modified-sawtooth-intkey'
...
def do_inc(args):
    name, value, receiver, wait = args.name, args.value, args.receiver, args.wait
    client = _get_client(args)
    response = client.inc(name, value, receiver, wait)
    print(response)
...
def do_dec(args):
    name, value, receiver, wait = args.name, args.value, args.receiver, args.wait
    client = _get_client(args)
    response = client.dec(name, value, receiver, wait)
    print(response)

def add_transfer_parser(subparsers, parent_parser):
    message = 'Sends an intkey transaction to transfer <value> from <name> to <receiver>.'

    parser = subparsers.add_parser(
        'transfer',
        parents=[parent_parser],
        description=message,
        help='transfers asset to receiver')
