...
class IntkeyMessageFactory:
    ...
    def _create_txn(self, txn_function, verb, name, value, receiver):
        payload = self._dumps({'Verb': verb, 'Name': name, 'Value': value, 'Receiver': receiver})

        addresses = [make_intkey_address(name)]

        return txn_function(payload, addresses, addresses, [])

    def create_tp_process_request(self, verb, name, value, receiver):
        txn_function = self._factory.create_tp_process_request
        return self._create_txn(txn_function, verb, name, value, receiver)

    def create_transaction(self, verb, name, value, receiver):
        txn_function = self._factory.create_transaction
        return self._create_txn(txn_function, verb, name, value, receiver)

    def create_batch(self, triples):
        txns = [
            self.create_transaction(verb, name, value, receiver)
            for verb, name, value, receiver in triples
        ]
